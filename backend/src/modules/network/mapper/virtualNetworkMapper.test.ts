...
describe('VirtualNetworkMapper', () => {
  it('should map nodeIds to virtual endpoints with correct container names', () => {
    ...
    expect(endpoints[0]).toEqual({
      ...
      containerName: 'torollo-lab-test-project-ubuntu-node-1'
    });
    expect(endpoints[1]).toEqual({
      nodeId: 'postgres_db',
      projectId: 'test-project',
      containerName: 'torollo-lab-test-project-postgres_db'
    });
    expect(endpoints[2]).toEqual({
      nodeId: 'nosql-3-node',
      projectId: 'test-project',
      containerName: 'torollo-lab-test-project-nosql-3-node'
    });
  });
  ...
  it('should sanitize characters correctly to generate valid container names', () => {
    ...
    const endpoints = VirtualNetworkMapper.mapNodesToEndpoints(projectId, nodeIds);
    expect(endpoints[0].containerName).toBe('torollo-lab-p-123-node123');
    expect(endpoints[1].containerName).toBe('torollo-lab-p-123-node_nametest');
  });
});
